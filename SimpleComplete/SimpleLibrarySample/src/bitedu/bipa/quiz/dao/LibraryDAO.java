...
public class LibraryDAO {
	...
	public boolean insertBookList(ArrayList<BookUseStatusVO> bookList) {
		boolean flag = false;
		
		String sql = "insert into book_use_status (book_seq, user_id, borrow_start, borrow_end, return_date)"
				+ "values (?, ?, ?, ?, ?)";
		
//		System.out.println(bookList.get(0).getBookSeq());
		try {
			Connection con = manager.getConnection();
			PreparedStatement pstmt = con.prepareStatement(sql);
			for (int i = 0; i < bookList.size(); i++) {
				pstmt.setInt(1, bookList.get(i).getBookSeq());
				pstmt.setString(2, bookList.get(i).getUserId());
				pstmt.setTimestamp(3, bookList.get(i).getBorrowStart());
				pstmt.setTimestamp(4, bookList.get(i).getBorrowEnd());
				pstmt.setTimestamp(5, bookList.get(i).getReturnDate());
