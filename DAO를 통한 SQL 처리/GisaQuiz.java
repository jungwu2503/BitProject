...
public class GisaQuiz {
	...
	public String solveQuiz4() {
		...
		StringBuffer sb = new StringBuffer("select count(kor) from (select kor, case loc_code when \"A\" then 5 ");
		sb.append("when \"B\" then 10 else 15 end as score from gisatbl where loc_code = \"A\" ");
		sb.append("or loc_code = \"B\") as t where (kor+score) > 50");
		String sql = sb.toString();
		System.out.println(sql);
